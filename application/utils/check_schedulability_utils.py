...
def is_schedulable(i, j, s, t, sta_specs):
    return is_station_check(s, j, sta_specs) and is_station_airbus(s, i) and is_station_date(s, t) and is_check_airbus(j, i)


# check if station s has capability for check j (set S_j)
def is_station_check(s, j, sta_specs):
    return sta_specs[s][j] > 0
